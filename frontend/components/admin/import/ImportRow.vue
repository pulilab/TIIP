...
export default {
  ...
  computed: {
    ...
    firstDHI() {
      if (
        this.dhi &&
        this.dhi[0].subGroups[0] &&
        this.dhi[0].subGroups[0].strategies
      ) {
  ...
  methods: {
      ...
      if (!this.valid) {
        ...
        if (container.scrollTop) {
          container.scroll(
            ...
            container.scrollTop - header.clientHeight
          )
        }
      }
    },
    ...
    async save(country, donor, publish, office) {
      ...
      const donorCustom = this.$children
        .filter((sc) => sc.type && sc.type.startsWith('INV'))
        .map((c) => ({
          donor_id: donor,
          question_id: this.customFieldsLib[c.type].id,
          answer: c.apiValue(),
